...
This talk builds the Phoenix application from a Dockerfile. An optional corporate CA certificate is trusted via a BuildKit secret (`--secret id=netskope_cert,src=certs/netskope.crt`) before dependency restore runs. The cert is never written to any image layer — omit `--secret` when not behind a TLS-intercepting proxy.
