...
- **Comparison Dockerfile** — the only Dockerfile in this talk is `comparison/Dockerfile.ruby`. The demo builds it with `ruby-app/` as the Docker build context, so the default `EXTRA_CERTS_DIR=certs` resolves to `ruby-app/certs/`. Put PEM-encoded `.crt` files there for that comparison build; the talk-root `certs/` folder remains the course-level placeholder.
