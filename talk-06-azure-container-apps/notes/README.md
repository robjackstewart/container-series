...
Both Dockerfiles trust an optional corporate CA certificate via a BuildKit secret (`--secret id=netskope_cert,src=certs/netskope.crt`) before Maven resolves dependencies. The cert is never written to any image layer — omit `--secret` when not behind a TLS-intercepting proxy.

The Maven builds use keytool to import the cert into the JDK `cacerts` keystore and remove it within the same RUN — the correct Java CA trust path (unlike the OpenSSL system store that Java ignores). The runtime images are JRE images rather than shell-less distroless images, so no distroless bundle copy is needed. If outbound HTTPS calls are added later, keep the system store updated or explicitly set `-Djavax.net.ssl.trustStore` to the intended trust store.
