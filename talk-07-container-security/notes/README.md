...
Each Dockerfile trusts an optional corporate CA certificate via a BuildKit secret (`--secret id=netskope_cert,src=certs/netskope.crt`) — the cert is never written to any image layer. Omit `--secret` when not behind a TLS-intercepting proxy.

The hardened and Chainguard final images are shell-less and do not need a runtime CA bundle for this demo — the `COPY --from=build … ca-certificates.crt` lines have been removed. If this application later makes outbound TLS calls from Node.js, `NODE_EXTRA_CA_CERTS` can point Node at a bundle supplied at runtime when the platform does not automatically use the system trust store.

> Expert aside: The certificate needs to be trusted where network I/O happens. For this talk that is the build stage running `npm ci` or `npm install`; the build secret ensures trust without leaving a trace in any layer.
