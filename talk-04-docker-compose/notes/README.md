...
This talk uses Dockerfiles for the API and Nginx builds. The Go build stage trusts optional corporate CA certificates copied into `certs/` through the `EXTRA_CERTS_DIR` build argument before `go mod download` runs. The files must be PEM-encoded `.crt` files; if a proxy exports `.pem`, rename it to `.crt` after confirming it is PEM text.

The final API image is shell-less, but the demo app only talks to PostgreSQL, Redis, and its own local health endpoint, so it does not copy a runtime CA bundle. If the Go service later makes outbound HTTPS calls, copy `/etc/ssl/certs/ca-certificates.crt` from the build stage into the final stage. Image pulls for PostgreSQL, Redis, Ollama, and Open WebUI use the Docker host or Docker Desktop trust store, so a registry x509 error during pull may need host-level proxy trust as well as the per-build `certs/` folder.
