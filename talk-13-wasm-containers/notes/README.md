...
This talk has two certificate paths. The traditional .NET Dockerfile uses the `EXTRA_CERTS_DIR` pattern in its build stage before `dotnet restore`, so the corporate CA must be copied into `traditional-app/certs/` when building with `traditional-app/` as the Docker build context. Keep the files PEM-encoded with a `.crt` extension; if Netskope exports a `.pem`, rename it to `.crt` after confirming it is PEM text.
