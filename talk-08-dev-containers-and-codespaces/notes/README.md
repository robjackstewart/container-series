...
This talk includes both image-based and Dockerfile-backed dev containers. The Python project uses a prebuilt dev container image and Features, so certificate trust depends on the host, Codespaces, and Feature resolution path. The TypeScript and full-stack examples build Dockerfiles that trust an optional corporate CA via a BuildKit secret (`--secret id=netskope_cert,src=<path>/netskope.crt`) before network work such as `apt`, `curl`, `npm`, or `go mod download` — the cert is never written to any image layer.

Behind Netskope or another TLS-intercepting proxy, save `netskope.crt` in the cert folder that belongs to the relevant build context and pass `--secret id=netskope_cert,src=<path>/netskope.crt`. Omit the secret when not behind a TLS-intercepting proxy.
