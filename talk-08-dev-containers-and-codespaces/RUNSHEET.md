...
6. The Compose config output lets you point at `build.args.EXTRA_CERTS_DIR`, the API service, the database service, volumes, and ports without opening YAML by hand.
...
## If it breaks
- **Build can't pull / x509 / TLS error** → Netskope or corporate TLS interception is not trusted: copy the corporate root CA as a PEM `.crt` into the cert folder for that build context, then rebuild. A dev container is itself a Docker build, so this matters before `apt`, `curl`, `npm`, or `go mod download`.
- **`COPY certs/` fails** → ensure the placeholder exists in the build context: `typescript-project\.devcontainer\certs\.gitkeep` for the TypeScript dev container and `fullstack-project\api\certs\.gitkeep` for the Go API Compose build.
