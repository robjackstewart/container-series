...
This talk has one Dockerfile. The Gradle build stage is Debian-based and trusts optional corporate CA certificates copied from `certs/` through the `EXTRA_CERTS_DIR` build argument before Gradle resolves dependencies or builds the jar. The files must be PEM-encoded `.crt` files; if Netskope exports `.pem`, rename it to `.crt` after confirming it is PEM text.
